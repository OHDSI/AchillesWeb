define(["jquery", "d3", "jnj/chart", "common", "datatables", "datatables-colvis"], function ($, d3, jnj_chart, common) {
	...
	$(document).on('click', '#measurement_table tbody tr', function () {
		...
		measurement_report.drilldown(id, concept_name);
	});
	...
	measurement_report.drilldown = function (concept_id, concept_name) {
		...
		$.ajax({
			type: "GET",
			url:getUrlFromDataCollection(page_vm.datasource(), "measurements", concept_id),
			success: function (data) {
				...
				ageAtFirstOccurrence.render(bpseries, "#reportMeasurements #ageAtFirstOccurrence", 500, 300, {
					xLabel: 'Gender',
				...
				prevalenceByMonth.render(byMonthSeries, "#reportMeasurements #measurementPrevalenceByMonth", 1000, 300, {
				...
				measurementsByType.render(common.mapConceptData(data.MEASUREMENTS_BY_TYPE), "#reportMeasurements #measurementsByType", 500, 300, {
					margin: {
						top: 5,
						left: 5,
						right: 220,
				...
				chart.render(dataByDecile, "#reportMeasurements #trellisLinePlot", 1000, 300, {
				...
				recordsByUnit.render(datdaRecordsByUnit, "#reportMeasurements #recordsByUnit", 500, 300, {
					margin: {
						top: 5,
						left: 5,
						right: 200,
				...
				bpseries = [];
				bpdata = common.normalizeDataframe(data.MEASUREMENT_VALUE_DISTRIBUTION);
				...
				measurementValues.render(bpseries, "#reportMeasurements #measurementValues", 500, 300, {
				...
				lowerLimit.render(bpseries, "#reportMeasurements #lowerLimit", 300, 200, {
					yMax: d3.max(bpdata.P90_VALUE) || bpdata.P90_VALUE, // handle when dataframe is not array of values
					xLabel: 'Unit',
					yLabel: 'Measurement Value'
				});
				...
				upperLimit.render(bpseries, "#reportMeasurements #upperLimit", 300, 200, {
					yMax: d3.max(bpdata.P90_VALUE) || bpdata.P90_VALUE, // handle when dataframe is not array of values
					xLabel: 'Unit',
					yLabel: 'Measurement Value'
				});							
				...
				relativeToNorm.render(dataRelativeToNorm, "#reportMeasurements #relativeToNorm", 500, 300, {
					margin: {
						top: 5,
						left: 5,
						right: 200,
	...
	measurement_report.render = function (datasource) {
		format_pct = d3.format('.2%');
		format_fixed = d3.format('.2f');
		format_comma = d3.format(',');

		$('#reportMeasurements svg').remove();
		...
		$.ajax({
			...
			success: function (data) {
				...
				$('#reportMeasurements').show();

				tree = buildHierarchyFromJSON(data, threshold);
				var treemap = new jnj_chart.treemap();
				treemap.render(tree, '#reportMeasurements #treemap_container', width, height, {
					onclick: function (node) {
						measurement_report.drilldown(node.id, node.name)
					},
